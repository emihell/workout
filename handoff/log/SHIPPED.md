...
## req-130 — our own exercise library (free-db copy + aliases + muscle groups + photo links) + RepDB credit (DEC-060)  (merged 2026-09-24)

Closeout 2026-09-24 (Builder session, branch `ff3dc9e`…`5485bc0`). Search reads `src/library/exercises.json` (879 = 876 free-db
@`a859101` + 2 extras + `own-hanging-knee-raise`), generated by `scripts/build-library.mjs` (sha256-checked) with a drift test;
RepDB stays a live fetch; nothing RepDB committed (`grep -ci repdb` → 0). Add-from-library saves optional `libraryId`
(`exercise-names.js`), no bump/rewrite; resolver `libraryEntryFor`. Credit on Settings + Search (`ExternalLink` ui primitive).
Spec review (10 findings folded in pre-build). Code review (independent subagent): no stored record can be rewritten; found a
single-word ranking regression from multi-word aliases ("press" → Dip Machine) → fixed (partial alias hit scores 2.5, below name
hits), plus no caching a library-less catalog, plus a behavioural rename test (`patchExercise`, same merge). Gate: Planner's own run
in an archive copy → `# tests 638 # pass 638 # fail 0`, build ✓ (library chunk 1,138.99 kB / 179.30 kB gzip, >500 kB warning
...
branch smoke green 12. No trigger files. Merge `54b9d62`. `reports/req-172.md`.

## req-173 — a quiet "No weight entered" note on a weighted set  (merged 2026-09-25)

**req-173** (2026-09-25) — "No weight entered" under a weighted set's blank kg (SetLogForm, `.ui-field-note`); Complete
