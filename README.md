...
has the boot prompts and the loop. In short: a Claude Code "planning" session writes
specs into `handoff/` (read-only to the builder, enforced by a pre-commit hook), a
Claude Code build session builds each `req-N` on its own branch, and `./plan`
publishes/merges/closes.
