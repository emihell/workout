...
describe('coverage', () => {
  it('every common entry has all four fields and passes every text rule', () => {
    assert.equal(common.length, 298) // req-140 (sanctioned): 178 + batch 1's 40; req-145/147 (DEC-074 §1): + batches 2 and 3
...
describe('unilateral and duplicates', () => {
  it('a unilateral entry names the other side in its steps', () => {
    ...
    assert.equal(oneSided.length, 53) // req-147 (DEC-074 §1): + 13 of batch 3; req-140 (sanctioned): + split squat, cable kickback, pistol, DB snatch; req-145 (sanctioned): + 16 of batch 2, + Dumbbell Step-Up (coach review)
  })
