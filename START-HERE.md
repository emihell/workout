...
### Planning (Claude Code)   ← this cold-starts the planning session
```
...
1. **Plan** (planning) — shape the req. Planning writes `handoff/work/req-N.md`,
...
3. **Record & close** (planning, *"process and close req-N"*) — planning reads the
...
- A req is never marked `BUILT AND MERGED` until git shows it on `main`. No
  optimistic pre-flip.
- Cross-worktree, use read-only plumbing only (`log`/`rev-parse`/`merge-base`/
  `show`) — never index-touching git (`status`/`checkout`/`merge`/`add`/`commit`)
  against another worktree's checkout: an interrupted command can leave a stale
  `.git/index.lock` that blocks it. (Planning's own `save`/`closeout` run from the
  planning worktree and clear the lock first.)
